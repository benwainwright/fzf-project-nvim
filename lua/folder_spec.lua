...
local function mock_popen_success(command)
    local handle = {}
    function handle:read(mode) return "mock output" end
    function handle:close() return true end
    return handle
end

local function mock_popen_nil(command) return nil end

local function mock_popen_handle_read_nil(command)
    local handle = {}
    function handle:read(mode) return nil end
    function handle:close() return true end
    return handle
end
...
local function mock_popen_no_output(command)
    local handle = {}
    function handle:read(mode) return "" end
    function handle:close() return true end
    return handle
end
...
describe("Folder class", function()
    ...
    after_each(function() io.popen = original_popen end)
    ...
    it("should error when command is nil", function()
        ...
        assert.has_error(function() Folder:new(params) end,
                         "'params.command' should have type 'string'")
    end)

    it("should error when path is nil", function()
        local params = {path = nil, command = "ls "}
        assert.has_error(function() Folder:new(params) end,
                         "'params.path' should have type 'string'")
    end)

    it("should error when params is nil", function()
        assert.has_error(function() Folder:new(nil) end,
                         "'params' should have type 'table'")
    end)
